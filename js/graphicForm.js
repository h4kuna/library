(function($){
	...
	$.fn.imageTick=function(options,disable){
		var defaults={
			...
			img_html:'<a href="#" onkeypress="$(\'img\', this).trigger(\'click\')" onclick="return false;"><img src="%s1" alt="no_tick" class="%s2" id="tick_img_%s3" /></a>',
			custom_button:false,
		...
		function handleClickType(using_custom_button,type,$input_id){
			...
			if(using_custom_button){
				...
				if(type=='checkbox'){
					...
					$("."+opt.image_tick_class+$input_id.attr('name')).attr('src',opt.no_tick_image_path);
		...
		this.each(function(){
			...
			if(using_custom_btn){
				...
				path = !!$obj.attr('checked')? opt.tick_image_path: opt.no_tick_image_path;
				img_html=opt.img_html.replace('%s1',path).replace('%s2',opt.image_tick_class+name).replace('%s3',id);
			}
			...
			$img_id.bind('clear', function(event){
				handleClickType.call(this,using_custom_btn,type,$input_id);
			})
